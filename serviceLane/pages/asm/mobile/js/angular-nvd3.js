...
(function(){
    ...
    angular.module('nvd3', [])
        ...
        .directive('nvd31', [function(){
            return {
                restrict: 'E',
                scope: {
                    data: '=',      //chart data, [required]
					height: '=',      //chart data, [required]
                ...
                link: function(scope, element, attrs){
                    ...
                    scope.api = {
                        ...
                        updateWithData: function (data){
                            if (data) {
                                ...
                                d3.select('#firstBar').select('svg').remove();
                                // Select the current element to add <svg> element and to render the chart in
                                d3.select('#firstBar').append('svg')
                                    .attr('height', scope.height)
                                    //.attr('width', scope.options.chart.width)
                                    .datum(data)
                                    .transition().duration(scope.options.chart['transitionDuration'])
                                    .call(scope.chart);
                                d3.select('#firstBar').select('svg')[0][0].style.height = scope.height + 'px';
								setTimeout(function(){scope.api.updatLine();},10);														
						...
						updatLine: function(){							
							a = d3.select(d3.selectAll(".tick")[0][1].getElementsByTagName("text")[0]).text();
							b = d3.selectAll(".tick")[0][1].getAttribute("transform");
							//var i = a.split(",");
							c = a.split("k")[0]+"000";
							x = b.split("(");
							y= x[1].split(",");
							if(scope.mileage >= 165000)
								scope.mileage = 165000
							z = (scope.mileage/parseInt(c)) * parseFloat(y[0]);
							d3.select('#firstBar').select('svg g').append('rect').attr("class","nv-indexLine").attr("height",(scope.data[0].totalRowCount *  27.875)+12).attr("transform","translate("+z+", -10)").attr("fill","#424345").attr("width",1);
							d3.select('#firstBar').select('svg g').append("polygon").attr("fill","#424345").attr("transform","translate("+(z-5)+", -13)").attr("points","0, 0 10, 0 5, 5");	
						},
                        
						refreshLine: function(){
							a = d3.selectAll(".tick")[0][0].getAttribute("transform");
							b= d3.selectAll(".tick")[0][1].getAttribute("transform");
							x = b.split("(");
							y= x[1].split(",");
							z = (scope.mileage/c) * y[0];
							d3.select('.nv-indexLine').attr("transform","translate("+z+", -10)").attr("fill","#81868e");
							d3.select("polygon").attr("fill","#81868e").attr("transform","translate("+(z-5)+", -13)");
						},
					...
					//});
				//}				
					scope.$watch('refreshLine', function(value) {
						if(value){
							scope.api.refreshLine();
							scope.refreshLine = false;
						}							
					});
					scope.$on('refreshMileageLine', function(event, updatedData) {						
						scope.mileage = updatedData.mileage;
						scope.data = updatedData.data;	
						scope.api.refresh();						
					});
				}				
			}
        }]).directive('nvd32', [function(){
            return {
                restrict: 'E',
                scope: {
                    data: '=',      //chart data, [required]
                    ...
                    scope.api = {
                        ...
                        updateWithData: function (data){
                            if (data) {
                                ...
                                scope.options.chart['transitionDuration'] = +scope.options.chart['transitionDuration'] || 250;
                                // remove whole svg element with old data
                                d3.select('#secondBar').select('svg').remove();
                                // Select the current element to add <svg> element and to render the chart in
                                d3.select('#secondBar').append('svg')
                                    .attr('height', scope.height)
                                    //.attr('width', scope.options.chart.width)
                                    .datum(data)
                                    .transition().duration(scope.options.chart['transitionDuration'])
                                    .call(scope.chart);
                                d3.select('#secondBar').select('svg')[0][0].style.height = scope.height + 'px';
								setTimeout(function(){scope.api.updatLine();},10);													
                                if (scope.options.chart.type === 'multiChart') scope.chart.update(); // multiChart is not automatically updated
						...
						updatLine: function(){
							...
							if(scope.mileage >= 165000)
								scope.mileage = 165000
							z = (scope.mileage/parseInt(c)) * parseFloat(y[0]);							
							d3.select('#secondBar').select('svg g').append('rect').attr("class","nv-indexLine").attr("height",(scope.data[0].totalRowCount *  27.875)+12).attr("transform","translate("+z+", -10)").attr("fill","#424345").attr("width",1);
							d3.select('#secondBar').select('svg g').append("polygon").attr("fill","#424345").attr("transform","translate("+(z-5)+", -13)").attr("points","0, 0 10, 0 5, 5");	
						},
				...
				//}				
					...
					scope.$on('refreshMileageLine2', function(event, updatedData) {					
						scope.mileage = updatedData.mileage;
						scope.data = updatedData.data;	
						scope.api.refresh();						
					});
