    ...
    selection.each(function(data) {
      ...
      switch (axis.orient()) {
        ...
        case 'bottom':
          ...
          axisLabel
              .attr('text-anchor', 'middle')
              .attr('y', xLabelMargin + 7) //Mileage lable y coodinate
              .attr('x', w/2);
          if (showMaxMin) {
...
nv.models.multiBarHorizontal = function() {
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      y   .domain([0, 170000]) //x axis label value
      ...
      if (showValues && !stacked) { //bar value by asm
        bars.select('text')
        ...
        bars.transition()
          .select('text')
            .attr('x', function(d,i) {  //bar coordination by asm
				if(getY(d,i) == 1000000)
					return getY(d,i) < 0 ? -4 : y(170000) - y(0) - 60;
				else if(getY(d,i) > 150000)
					return getY(d,i) < 0 ? -4 : y(getY(d,i)) - y(0) + 4 - 45
				else
					return getY(d,i) < 0 ? -4 : y(getY(d,i)) - y(0) + 4;
				/*if(getY(d,i) == 1000000)
					return getY(d,i) < 0 ? -4 : y(170000) - y(0) + 4 - 70			
				else if(getY(d,i) <= 80000)
					return getY(d,i) < 0 ? -4 : y(getY(d,i)) - y(0) + 4 - 40
				else if(getY(d,i) <= 170000)
					return getY(d,i) < 0 ? -4 : y(getY(d,i)) - y(0) + 4 - 45
				else if(getY(d,i) > 170000)
					return getY(d,i) < 0 ? -4 : y(170000) - y(0) + 4 - 45*/
      ...
      if (stacked)
        ...
        bars.transition()
            .attr('transform', function(d,i) {
              ...
              return 'translate(' +
              (getY(d,i) < 0 ? y(getY(d,i)) : 1)
              + ',' +
              //(d.series * x.rangeBand() / data.length + x(getX(d,i)) )
			  ((getX(d,i) * 28)) //bar gap by asm
			  + ')'
            })
          .select('rect')
            .attr('height', 25 )
			...
			if(getY(d,i) <= 170000)
              return Math.max(Math.abs(y(getY(d,i)) - y(0)),1)
				...
				return Math.max(Math.abs(y(170000) - y(0)),1)
            });
...
nv.models.multiBarHorizontalChart = function() {
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      var availableWidth = (width  || parseInt(container.style('width')) || 960) + 40,
          availableHeight = (height || parseInt(container.style('height')) || 400)
                             - margin.top - margin.bottom;		
      chart.update = function() { container.transition().duration(transitionDuration).call(chart) };
      chart.container = this;
	...
	setTimeout(function(){
      ...
      if (showYAxis) {		  			
          yAxis
            .scale(y)
            .ticks( availableWidth / 100 ) //y axis distance 
            .tickSize( -availableHeight, 0);
