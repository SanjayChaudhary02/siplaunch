  ...
  var DatetimePickerCtrl = function($compile, $document) {
    ...
    this.openDatetimePicker = function(options) {
      ...
      if(bcr.width > datePickerElBcr.width){
        ...
        datetimePickerEl.style.left= (bcr.left + window.pageXOffset - 205) + 'px';
      }
    ...
    this.closeDatetimePicker = function(evt) {
  	...
  	$(window).resize(function(){
		if(document.querySelector('div[datetime-picker-popup]'))
		{
		document.querySelector('div[datetime-picker-popup]').parentNode.removeChild(document.querySelector('div[datetime-picker-popup]'));	
		document.body.removeEventListener('click', this.closeDatetimePicker);
		}
    });
  ...
  var datetimePickerPopup = function($locale, dateFilter){
    ...
    var linkFunc = function(scope, element, attrs, ctrl) { //jshint ignore:line
      ...
      scope.updateNgModel = function(day) {
		...
		var tempHour = new Date().getHours();
		var tempMin = new Date().getMinutes();
        scope.selectedDate = new Date(
          scope.mv.year, scope.mv.month, day, tempHour, tempMin, 0
        );
        scope.selectedDay = scope.selectedDate.getDate();
        if (attrs.ngModel) {
